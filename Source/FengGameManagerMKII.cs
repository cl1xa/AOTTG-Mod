...
internal class FengGameManagerMKII : Photon.MonoBehaviour
{
    ...
    private void Awake()
    {
        new Mod.Main(); //Personal init for modded objects
    ...
    //Modified
    [RPC]
    private void ChatPM(string sender, string content, PhotonMessageInfo info = null)
    {
        if (content.StripHTML().Length > 300)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Large private message");
            return;
        }
        ...
        chatRoom.addLINE($"<color=#FFC000><b>(PM)</b> [{info.sender.ID}]</color> <color=white>{sender}: {content}</color>");
    }
    ...
    private void getRacingResult(string player, float time, PhotonMessageInfo info)
    {
        try
        {
            if (IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.RACING)
            {
                if (info != null)
                {
                    Mod.NetworkHandler.HandleOperationResponse(info.sender, "Racing exploit or invalid getRacingResult parameters");
                    return;
                }
            }
            else
            {
                if (time > (float)int.MaxValue)
                {
                    Mod.NetworkHandler.HandleOperationResponse(info.sender, $"Invalid getRacingResult > time:{time}");
                    return;
                }
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, $"Invalid getRacingResult > p:{player} | t:{time}");
            return;
        }
    }
    ...
    //Modified
    //This is when you are not the host, and the MC bans someone else
    //As a result of the masterclient ignoring someone, you will ignore them as well.
    [RPC]
    private void ignorePlayer(int ID, PhotonMessageInfo info)
    {
        if (info.sender.isMasterClient)
        {
            if (ID.Equals(PhotonNetwork.player.ID))
                Mod.DebugConsole.Message(Mod.DebugConsole.MessageType.warning, "You were kicked/ignored by the MasterClient");
            ...
            if (photonPlayer != null && !Mod.NetworkHandler.IsPlayerIgnored(photonPlayer))
            {
                for (int i = 0; i < PhotonNetwork.playerList.Length; i++)
                {
                    ...
                    {
                        Mod.NetworkHandler.ignoreList.Add(ID);
    ...
    //This is when you are not the host, and the MC bans someone else
    //As a result of the masterclient ignoring someone, you will ignore them as well
    //The masterclient stores an array of blacklisted IDs, in case for whatever reason someone is bypassing
    //the master kick and remains in the server. This RPC prevents those modders from effecting new joiners
    [RPC]
    private void ignorePlayerArray(int[] IDS, PhotonMessageInfo info)
    {
        if (info.sender.isMasterClient)
        {
            Mod.DebugConsole.Message(Mod.DebugConsole.MessageType.log, "[Banned IDs]:" + string.Join(", ", IDS.Select((int x) => x.ToString()).ToArray()));
            ...
            foreach (int num in IDS)
            {
                ...
                for (int j = 0; j < PhotonNetwork.playerList.Length; j++)
                {
                    ...
                    {
                        Mod.NetworkHandler.ignoreList.Add(num);
    ...
    public void justRecompileThePlayerList()
    {
        ...
        if (SettingsManager.LegacyGameSettings.TeamMode.Value != 0)
        {
            ...
            foreach (PhotonPlayer photonPlayer in array)
            {
                if (photonPlayer.customProperties[PhotonPlayerProperty.dead] != null && !Mod.NetworkHandler.IsPlayerIgnored(photonPlayer))
                {
            ...
            foreach (PhotonPlayer value in dictionary.Values)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(value))
                {
                    text += "[FF0000][X] ";
                }
            ...
            foreach (PhotonPlayer value2 in dictionary2.Values)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(value2))
                {
                    text += "[FF0000][X] ";
                }
            ...
            foreach (PhotonPlayer value3 in dictionary3.Values)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(value3))
                {
                    text += "[FF0000][X] ";
                }
        ...
        else
        {
            ...
            foreach (PhotonPlayer photonPlayer4 in array)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(photonPlayer4))
                {
                    text += "[FF0000][X] ";
                }
        ...
        if (SettingsManager.LegacyGameSettings.InfectionModeEnabled.Value)
        {
            ...
            for (int j = 0; j < PhotonNetwork.playerList.Length; j++)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(photonPlayer5) || photonPlayer5.customProperties[PhotonPlayerProperty.dead] == null || photonPlayer5.customProperties[PhotonPlayerProperty.isTitan] == null)
                {
                    continue;
                }
        ...
        else
        {
            ...
            if (SettingsManager.LegacyGameSettings.TeamMode.Value > 0 && PhotonNetwork.playerList.Length > 1)
            {
                ...
                for (int j = 0; j < PhotonNetwork.playerList.Length; j++)
                {
                    ...
                    if (Mod.NetworkHandler.IsPlayerIgnored(photonPlayer7) || photonPlayer7.customProperties[PhotonPlayerProperty.RCteam] == null || photonPlayer7.customProperties[PhotonPlayerProperty.dead] == null)
                    {
                        continue;
                    }
    ...
    {
        if ((info != null && info.sender.isMasterClient) || (Extensions.IsTitan(info.sender) && info != null))
        {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netShowDamage parameters");
            return;
        }
    }
    ...
    //Modified
    [RPC]
    public void oneTitanDown(string name1, bool onPlayerLeave, PhotonMessageInfo info = null)
    {
        if (IN_GAME_MAIN_CAMERA.gametype != 0 && !PhotonNetwork.isMasterClient)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "oneTitanDown RPC without MasterClient");
            return;
        }
    ...
    //Modified
    //There was some poorly written code, I cleaned some of it up. Not all of it.
    public void OnGUI()
    {
        if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.STOP && Application.loadedLevelName != "characterCreation")
        {
            ...
            {
                legacyPopupTemplate.DrawPopup("<b>[LOADING GAME DATA]</b>", 200f, 22f);
            }
        ...
        else
        {
            ...
            else
            {
                ...
                if (Time.timeScale <= 0.1f)
                {
                    ...
                    //else //Patched, I hate the fact that I can't move when the game is paused
                    //{
    ...
    public void OnJoinedRoom()
    {
        ...
        //Second iteration
        //Mod.NetworkHandler.ApplyRandomProperty();
    }
    ...
    public void OnPhotonPlayerConnected(PhotonPlayer player)
    {
        Mod.DebugConsole.Message(Mod.DebugConsole.MessageType.blank,
          $"<color=lime><b>[JOIN]</b></color> [{player.ID}] {Extensions.GetPlayerName(player).HexColor()}");

        if (Mod.NetworkHandler.IsPlayerBanned(player))
        {
            Mod.NetworkHandler.HandleOperationResponse(player, "Previous infraction, player is blacklisted");
            return;
        }
        ...
        //I dont know if I need this code, considering this check exists in the event code where properties are sent...
        //Keeping it anyway just in case? Lol
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(player, "Excessive stats (2)");
            return;
        }
        ...
        if (PhotonNetwork.isMasterClient)
        {
            ...
            if (Mod.NetworkHandler.ignoreList != null && Mod.NetworkHandler.ignoreList.Count > 0)
            {
                photonView.RPC("ignorePlayerArray", player, Mod.NetworkHandler.ignoreList.ToArray());
            }
    ...
    public void OnPhotonPlayerDisconnected(PhotonPlayer player)
    {
        ...
        //Remove the banned ID from the array, they cant join back with the same ID
        //and their name is banhashed anyway lol
        if (Mod.NetworkHandler.IsPlayerIgnored(player))
            Mod.NetworkHandler.ignoreList.Remove(player.ID);

        InstantiateTracker.instance.TryRemovePlayer(player.ID);

        if (PhotonNetwork.isMasterClient)
        {
            //I think this is to prevent fake leaving? Or to ensure on all clients that the player has left
            //All it seems to is if if im the MC and someone leaves, I force everyone to ban hash that person when they leave?
            //I have no fucking idea what the fuck this is lol
            base.photonView.RPC("verifyPlayerHasLeft", PhotonTargets.All, player.ID);

            if (SettingsManager.LegacyGameSettings.PreserveKDR.Value)
            {
                string key = Extensions.GetPlayerName(player);
    ...
    public void OnPhotonPlayerPropertiesChanged(object[] playerAndUpdatedProps)
    {
        ...
        if (hashtable.ContainsKey("statACL") || hashtable.ContainsKey("statBLA") || hashtable.ContainsKey("statGAS") || hashtable.ContainsKey("statSPD"))
        {
            ...
            if (num > 150)
            {
                ExitGames.Client.Photon.Hashtable hashtable2 = new ExitGames.Client.Photon.Hashtable { { PhotonPlayerProperty.statACL, 100 } };
                PhotonNetwork.player.SetCustomProperties(hashtable2);
                num = 100;
            }
            if (num2 > 125)
            {
                ExitGames.Client.Photon.Hashtable hashtable2 = new ExitGames.Client.Photon.Hashtable { { PhotonPlayerProperty.statBLA, 100 } };
                PhotonNetwork.player.SetCustomProperties(hashtable2);
                num2 = 100;
            }
            if (num3 > 150)
            {
                ExitGames.Client.Photon.Hashtable hashtable2 = new ExitGames.Client.Photon.Hashtable { { PhotonPlayerProperty.statGAS, 100 } };
                PhotonNetwork.player.SetCustomProperties(hashtable2);
                num3 = 100;
            }
            if (num4 > 140)
    ...
    public void pauseRPC(bool pause, PhotonMessageInfo info)
    {
        if (!info.sender.isMasterClient)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "pauseRPC without MasterClient");
            return;
        }

        if (Mod.Features.blockPause)
            return;
    ...
    {
        ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable { { PhotonPlayerProperty.kills, (int)player.customProperties[PhotonPlayerProperty.kills] + 1 } };
        player.SetCustomProperties(hashtable);
        hashtable = new ExitGames.Client.Photon.Hashtable { { PhotonPlayerProperty.max_dmg, Mathf.Max(dmg, (int)player.customProperties[PhotonPlayerProperty.max_dmg]) } };
        player.SetCustomProperties(hashtable);
    ...
    private void resetSettings(bool isLeave)
    {
        ...
        Mod.NetworkHandler.banHash = new ExitGames.Client.Photon.Hashtable();
        Mod.NetworkHandler.ignoreList = new List<int>();
    }
    ...
    private void RPCLoadLevel(PhotonMessageInfo info)
    {
        if (info.sender.isMasterClient)
        {
            ...
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Restart event without MasterClient"); //Should I return after this check? Default RC does not
        else
        {
            //Idk what this is?
            if (masterRC)
    ...
    private void setGameSettings(ExitGames.Client.Photon.Hashtable hash)
    {
        ...
        if (hash.ContainsKey("motd"))
        {
            if (legacyGameSettings.Motd.Value != (string)hash["motd"])
            {
                ...
                chatRoom.addLINE("<color=#FFCC00>MOTD:" + legacyGameSettings.Motd.Value.FixHTMLFormatting() + "</color>");
            }
        }
    ...
    //Modified
    [RPC]
    private void showResult(string text0, string text1, string text2, string text3, string text4, string text6, PhotonMessageInfo t, PhotonMessageInfo info)
    {
        ...
        else if ((!t.sender.isMasterClient && PhotonNetwork.player.isMasterClient) && info.sender != null)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "False game end or invalid showResult parameters");
            return;
        }
    }
    ...
    {
        if (player != null && info.sender != null && (Damage > 10000 || (!name.InRoom(player.ID) && name != "Titan" && name != "Jumper" && name != "Crawler" && name != "Aberrant" && name != "Punk" && name != "Colossal Titan" && name != "Female Titan")))
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid titanGetKill parameters");
            return;
        }
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, $"Invalid titanGetKill parameters | {ex.Message}");
            return;
        }
    }
    ...
    {
        if (!Extensions.IsTitan(info.sender) && !info.sender.isMasterClient && !victim.InRoom(info.sender.ID))
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, $"Invalid updateKillInfo parameters | t1:{t1} t2: {t2} k:{killer} v:{victim} dmg:{dmg}");
            return;
        }
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, $"Invalid updateKillInfo event");
            return;
        }
    ...
    //Modified
    //Optimized garbage code
    [RPC]
    public void verifyPlayerHasLeft(int ID, PhotonMessageInfo info)
    {
        PhotonPlayer photonPlayer = PhotonPlayer.Find(ID);

        if (info.sender.isMasterClient && photonPlayer != null)
            Mod.NetworkHandler.banHash.Add(ID, Extensions.GetPlayerName(photonPlayer));
    }
    ...
    public IEnumerator WaitAndRecompilePlayerList(float time)
    {
        ...
        if (SettingsManager.LegacyGameSettings.TeamMode.Value == 0)
        {
            ...
            foreach (PhotonPlayer photonPlayer in array)
            {
                ...
                if (Mod.NetworkHandler.IsPlayerIgnored(photonPlayer))
                {
                    text += "[FF0000][X] ";
                }
        ...
        else
        {
            ...
            foreach (PhotonPlayer photonPlayer2 in array)
            {
                if (photonPlayer2.customProperties[PhotonPlayerProperty.dead] != null && !Mod.NetworkHandler.IsPlayerIgnored(photonPlayer2))
                {
