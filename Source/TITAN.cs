...
internal class TITAN : Photon.MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (Mod.Features.titanESP)
        {
            Transform transform = base.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest");
            Vector3? vector = Camera.main.WorldToScreenPoint(transform.position);
            Vector3 value = vector.Value;

            if (vector.HasValue && value.z > 1f)
            {
	...
	public void grabbedTargetEscape(PhotonMessageInfo info)
	{
		...
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid grabbedTargetEscape parameters");
			return;
        }
	...
	public void grabToLeft(PhotonMessageInfo info)
	{
		...
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid grabToLeft parameters");
            return;
		}
	...
	public void grabToRight(PhotonMessageInfo info)
	{
		if (PhotonNetwork.isMasterClient && info != null && info.sender != base.photonView.owner)
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid grabToRight parameters");
            return;
        }
	...
	private void netSetLevel(float level, int AI, int skinColor, PhotonMessageInfo info)
	{
		...
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netSetLevel parameters");
            return;
        }
