...
public class InstantiateTracker
{
    ...
    public bool checkObj(string key, PhotonPlayer photonPlayer, int[] viewIDS)
    {
        ...
        foreach (int num3 in viewIDS)
        {
            ...
            {
                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning invalid photonView");
                return false;
            }
        }
        ...
        if (Features.logObjects)
        {
            string[] commonObjs = { "hook", "redcross", "hitmeat" };

            if (!commonObjs.Any(x => x.ToLower() == key))
                Mod.DebugConsole.Message(Mod.DebugConsole.MessageType.obj, $"[{photonPlayer.ID}] {Extensions.GetPlayerName(photonPlayer).HexColor()} | {key.ToLower()}");
        }
        ...
        switch (key)
        {
            ...
            case "rcasset/bombexplodemain":
                ...
                {
                    if (!FengGameManagerMKII.instance.restartingBomb)
                        Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning spawn without bomb gamemode");
            ...
            case "fx/bite":
                ...
                if (!FengGameManagerMKII.instance.restartingTitan)
                    Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning titan effects");
            ...
            case "rcasset/cannonground":
                if (!FengGameManagerMKII.instance.allowedToCannon.ContainsKey(photonPlayer.ID) && !FengGameManagerMKII.instance.restartingMC)
                    Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning cannon item without permission");

                return Instantiated(photonPlayer, GameResource.general);

            case "rcasset/cannonwallprop":
            case "rcasset/cannongroundprop":
                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, $"Spawning MasterClient object | object:{key}");
                return false;
            ...
            case "titan_eren":
                if (!(RCextensions.returnStringFromObject(photonPlayer.customProperties[PhotonPlayerProperty.character]).ToUpper() != "EREN"))
                {
                    ...
                    {
                        if (!FengGameManagerMKII.instance.restartingEren)
                            Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning Eren titan");
                ...
                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning Eren titan (2)");
                return false;
            ...
            case "hitmeatbig":
                if (!(RCextensions.returnStringFromObject(photonPlayer.customProperties[PhotonPlayerProperty.character]).ToUpper() != "EREN"))
                {
                    ...
                    {
                        if (!FengGameManagerMKII.instance.restartingEren)
                            Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning Eren effect (1)");
                        return false;
                    }
                    return Instantiated(photonPlayer, GameResource.effect);
                }

                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning Eren effect (2)");
                return false;
            ...
            case "fx/boom1_ct_kick":
                ...
                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning Colossal Titan effect");
                return false;
            ...
            case "horse":
                ...
                if (!FengGameManagerMKII.instance.restartingHorse)
                    Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning horse without permission");
            ...
            case "titan_ver3.1":
                ...
                else
                {
                    ...
                    {
                        Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning titans (1)");
                        return false;
                    }
                    ...
                    if (IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.BOSS_FIGHT_CT)
                    {
                        ...
                        if (num4 > 1)
                        {
                            Mod.NetworkHandler.HandleOperationResponse(photonPlayer, "Spawning titans (2)");
                            return false;
                        }
                    }
                }
            ...
            default:
                Mod.NetworkHandler.HandleOperationResponse(photonPlayer, $"Spawning unknown object | object:{key}");
                return false;
        }
    }
        ...
        if (TryGetPlayer(owner.ID, out var result))
        {
            if (players[result].IsThingExcessive(type))
            {
                if (owner != null && PhotonNetwork.isMasterClient)
                    Mod.NetworkHandler.HandleOperationResponse(owner, $"Spamming object | object:{type}");
