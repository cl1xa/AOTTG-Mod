...
internal class Bullet : Photon.MonoBehaviour
{
	...
	private void myMasterIs(int id, string launcherRef, PhotonMessageInfo info)
	{
        ...
        if ((info.sender != base.photonView.owner || id < 0) || photonView.owner != info.sender)
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid myMasterIs (bullet) parameters");
            return;
		} 
		else if (!(photonView == null) && !(photonView.gameObject == null))
		{
			...
			switch (launcherRef)
			{
                ...
                case "hookRefR2":
                    ...
                    break;
				default: //Idek if this will ever trigger but whatever I don't want to risk it?
                    Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid myMasterIs (bullet) parameters (2)");
                    return;
			}
		}
	}
	...
	private void setPhase(int value, PhotonMessageInfo info)
	{
		...
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid setPhase parameters");
            return;
        }
	...
	private void setVelocityAndLeft(Vector3 value, Vector3 v2, bool l, PhotonMessageInfo info)
	{
		if (info.sender != base.photonView.owner)
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid setVelocityAndLeft parameters");
            return;
		}
	...
	private void tieMeTo(Vector3 p, PhotonMessageInfo info)
	{
		if (info.sender != base.photonView.owner)
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid tieMeTo parameters");
			return;
        }
	...
	private void tieMeToOBJ(int id, PhotonMessageInfo info)
	{
		if (info.sender != base.photonView.owner)
		{
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid tieMeToOBJ parameters");
            return;
		}
