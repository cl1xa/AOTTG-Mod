...
internal class HERO : Photon.MonoBehaviour
{
    ...
    public void OnGUI()
    {
        if (GameMenu.Paused || !PhotonNetwork.connected || !PhotonNetwork.inRoom || PhotonNetwork.room.playerCount <= 1)
            return;
        ...
        foreach (GameObject gameObject in array)
        {
            ...
            if (gameObject != null && owner != null && gameObject.GetComponent<HERO>() != null && !gameObject.GetComponent<HERO>().hasDied && !owner.isLocal)
            {
                Vector3? vector = Camera.main.WorldToScreenPoint(gameObject.transform.Find("Amarture/Controller_Body/hip/spine/chest/neck/head").transform.position);
                if (vector.HasValue && vector.Value.z > 1f)
                {
                    ...
                    GUILayout.Label(RCextensions.returnStringFromObject(owner.customProperties[PhotonPlayerProperty.guildName]).ToString().HexColor() +
                        "\n" +
                        RCextensions.returnStringFromObject(owner.customProperties[PhotonPlayerProperty.name]).ToString().HexColor());
                    GUILayout.EndArea();
                }
            }
        }
    ...
    public void blowAway(Vector3 force, PhotonMessageInfo info)
    {
        if (info != null)
        {
            ...
            {
                Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid blowAway parameters");
                return;
            }
            ...
            {
                Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid blowAway parameters (2)");
                return;
            }
        }
    ...
    private void killObject(PhotonMessageInfo info)
    {
        if (info != null)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid killObject parameters");
            return;
        }
    }
    ...
    public void moveToRPC(float posX, float posY, float posZ, PhotonMessageInfo info)
    {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid moveToRPC call");
            return;
        }

        if (Mod.Features.blockMove)
        {
            Mod.DebugConsole.Message(Mod.DebugConsole.MessageType.warning, $"Blocked valid moveToRPC from MasterClient | {Extensions.GetPlayerName(info.sender)}");
            return;
        }
    ...
    private void net3DMGSMOKE(bool ifON, PhotonMessageInfo info)
    {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid net3DMGSmoke parameters");
            return;
        }
    ...
    private void netContinueAnimation(PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netContinueAnimation parameters");
            return;
        }
    ...
    private void netCrossFade(string aniName, float time, PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netCrossFade parameters");
            return;
        }
    ...
    private void DeflectBomb(PhotonPlayer player)
    {
        PreventDie();
        ...
        foreach (GameObject gameObject in array)
        {
            if (gameObject.GetPhotonView().owner == player)
                gameObject.GetComponent<HERO>().photonView.RPC("netDie2", player, -1, "revolutionary feature");
        }
    }
    ...
    public void netDie(Vector3 v, bool isBite, int viewID = -1, string titanName = "", bool killByTitan = true, PhotonMessageInfo info = null)
    {
        ...
        {
            if (Mod.Features.ignoreRacingRed)
            {
                if (SettingsManager.LegacyGameSettings.RacingEndless.Value || SettingsManager.LegacyGameSettings.RacingStartTime.Value > 0f)
                {
                    PreventDie();
                    return;
                }
            ...
            if (Mod.Features.deflectBomb)
            {
                if (SettingsManager.LegacyGameSettings.BombModeEnabled.Value)
                {
                    DeflectBomb(info.sender);
                    return;
                }
            }
            ...
            if (!info.sender.isLocal && !info.sender.isMasterClient)
            {
                ...
                {
                    Mod.NetworkHandler.HandleOperationResponse(info.sender, "Unusual kill");
                    return;
                }
                else if (viewID < 0)
                {
                    if (titanName == "")
                    {
                        Mod.NetworkHandler.HandleOperationResponse(info.sender, "Unusual kill (possible valid)");
                        PreventDie();
                        return;
                    }
                    else
        ...
        if (base.photonView.isMine && info != null && IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.BOSS_FIGHT_CT)
        {
            ...
            if (!info.sender.isLocal && !info.sender.isMasterClient)
            {
                ...
                else if (viewID < 0)
                {
                    if (titanName == "")
                    {
                        Mod.NetworkHandler.HandleOperationResponse(info.sender, "Unusual kill (possibly valid) (2)");
                        PreventDie();
                        return;
                    }
                }
    ...
    private void netGrabbed(int id, bool leftHand, PhotonMessageInfo info)
    {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netGrabbed parameters");
            return;
        }
    ...
    private void netlaughAttack(PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netLaughAttack parameters");
            return;
        }
    ...
    private void netPauseAnimation(PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netPauseAnimation parameters");
            return;
        }
        foreach (AnimationState item in base.animation)
        {
    ...
    private void netPlayAnimation(string aniName, PhotonMessageInfo info)
    {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netPlayAnimation parameters");
            return;
        }
    ...
    private void netPlayAnimationAt(string aniName, float normalizedTime, PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netPlayAnimationAt parameters");
            return;
        }
    ...
    private void netSetIsGrabbedFalse(PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netSetIsGrabbedFalse parameters");
            return;
        }
    ...
    private void netTauntAttack(float tauntTime, float distance, PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid netTaunt parameters");
            return;
        }
    ...
    //Modified, I hate that people can hook you around its stupid as shit
    [RPC]
    private void RPCHookedByHuman(int hooker, Vector3 hookPosition, PhotonMessageInfo info = null)
    {
        hookBySomeOne = false;
        badGuy = null;
    ...
    public void SetMyCannon(int viewID, PhotonMessageInfo info)
    {
        ...
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid SetMyCannon parameters (1)");
            return;
        }
        else if (PhotonView.Find(viewID).owner != info.sender)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid SetMyCannon parameters (2)");
            return;
        }
        else
        {
            if (info.sender != base.photonView.owner) //Double check? Idk lol
            {
                Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid SetMyCannon parameters (3)");
                return;
            }
    ...
    private void showHitDamage(PhotonMessageInfo info)
    {
        if (info != null)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid showHitDamage parameters");
            return;
        }
    }
    ...
    {
        if (Mod.Features.infiniteBlades)
            return;
    ...
    private void whoIsMyErenTitan(int id, PhotonMessageInfo info)
    {
        if (info != null && info.sender != base.photonView.owner)
        {
            Mod.NetworkHandler.HandleOperationResponse(info.sender, "Invalid whoIsMyErenTitan parameters");
            return;
        }
